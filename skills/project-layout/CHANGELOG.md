...
- 术语统一：「主题工作胶囊」全量更名为「事务胶囊」，与 `.system/rules/`（项目组织、文件交付、治理指令、角色协作、软件工程）、`front_matter.schema.json` 及体检工具同轮对齐，消除同一机制两个名字的版本漂移。
...
- 首次发布。从 `.system/rules/项目组织.md` 下沉结构清单与归位操作：主题工作胶囊目录树、4 域分层结构、`RawInput/` 整理映射表、代码工程 `docs/` 事件容器布局、`README.md` 总纲内容规范。
