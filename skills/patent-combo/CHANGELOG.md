...
### Security
- 禁止环境自检在运行期自动安装 Python、Cargo 或浏览器依赖；主依赖和 STEP 依赖改为精确版本，CAD 安装仅允许隔离模式访问 PyPI 官方源。
- 公式代算移除 `eval/compile`，改用白名单 AST 递归解释器。
- 环境自检新增内置 Mermaid 11.4.1 SHA-256 完整性门禁，资产缺失或哈希不匹配时阻断核心流程。
- CAD 虚拟环境清理增加目录归属和符号链接拒绝门禁。
- CNIPA 明文 HTTP 检索增加公开词确认参数、敏感特征与长度阻断、传输风险标记及结果二次核验要求。

## [1.4.5] - 2026-09-04

### Fixed
- 发明交底收尾前强制校验至少两张 Mermaid 图及“关键实现代码”中的脱敏代码或伪代码块，缺项时保留阶段底稿并阻断交付。
- 交底书转换改为先运行 `mermaid_render.py` 生成并核验 PNG，再由 `md_to_docx.py` 嵌入 Word，避免收敛器绕过出图路径。
...
### Changed
- 删除可分发 Skill 中对 `.data/templates/registry.md` 的间接定位；内置资产出处仅保留随包可验证事实，内部源定位改留受控维护记录。

## [1.3.1] - 2026-09-04

### Changed
- 内置资产出处泛化：可分发面不再携带工作区内部目录结构，源仓库路径改由 `.data/templates/registry.md`（`ip` 项目）间接定位。配套根系统铁律 6 澄清（公共权威数据源/开源许可署名/开源致谢/公共规范引用豁免）。
...
### Changed
- 实例配置收缩：`.data/skills/patent-combo/config.json` 仅必需 `output_root`；`priorart_cli` 与旧版 `disclosure_skill`/`claims_guide` 降为可选覆盖键（legacy 模式）。
- 单测 `tests/test_patent_combo_env.py`：6 例覆盖就绪 / 降级 / 阻断 / 双安装路径 / legacy 覆盖。

## [1.1.0] - 2026-09-03

### Changed
- 入册 Entropaxis（`.system/skills/patent-combo/`），三件套依赖路径全部外置至工作区实例声明 `.data/skills/patent-combo/config.json`，Skill 零路径硬编码，可随根系统分发。
