...
def _open_path(path_text: str, config: dict, config_reason: str, platform: str, runner) -> dict:
...
def open_paths(
    ...
    *,
    ...
    return [_open_path(path, config, config_reason, platform, runner) for path in paths]
    ...
    if args.json:
        ...
        for result in results:
            if result["ok"] and result["used_fallback"]:
                print(f"[WARN] 已使用系统默认程序打开 {result['path']}；原因: {result['reason']}")
            elif result["ok"]:
                print(f"[OK] 已使用 {result['selected_app']} 打开 {result['path']}")
            else:
