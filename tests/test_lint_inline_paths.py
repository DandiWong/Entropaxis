...
`python3 .system/tools/init_capsule.py`），它不是 Markdown 链接。只扫 `](...)` 会让
...
class InlineCodePathLintTests(unittest.TestCase):
    def test_missing_system_tool_is_blocking(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            _mk_workspace(root, "执行 `python3 .system/tools/ghost.py <父目录>` 生成骨架。")
    ...
    def test_existing_system_tool_passes(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            _mk_workspace(root, "执行 `python3 .system/tools/real.py` 生成骨架。")
            (root / ".system" / "tools").mkdir(parents=True, exist_ok=True)
            (root / ".system" / "tools" / "real.py").write_text("", encoding="utf-8")
    ...
    def test_missing_data_declaration_is_advisory_not_blocking(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            _mk_workspace(root, "参数见 `.data/rules/ghost-config.md`。")
            self.assertEqual(check_system_markdown_links(root), [])
    ...
    def test_placeholder_and_glob_forms_are_ignored(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            _mk_workspace(
                root,
                "新建 `.system/tools/<name>.py`；映射 `.data/templates/X` ⟺ "
                "`.system/templates/X.template.*`；凭证放 `.data/credentials/<skill-name>/`。",
            )
            self.assertEqual(check_system_markdown_links(root), [])
    ...
    def test_plain_prose_path_outside_backticks_is_ignored(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            _mk_workspace(root, "系统研究产物归位 .system/tools/ghost.py 所在层级。")
            self.assertEqual(check_system_markdown_links(root), [])
