...
Skill 要能脱离本工作区、以任意方式安装后独立运行。`.system/tools/x.py` 在只装了
一个 Skill 的机器上根本不存在——把它写成无条件前置，Skill 换个地方就跑不起来。
同时 `open_file.py` 仍由 `.system/` 单点掌管：允许"有就调、没有就降级"，
...
class SkillIndependenceTests(unittest.TestCase):
    def test_unconditional_system_tool_call_is_blocked(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            _mk_skill(root, "demo", '交付前执行 `python3 .system/tools/open_file.py "<path>"`。')
    ...
    def test_optional_marker_on_same_line_passes(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            _mk_skill(root, "demo", "若工作区提供 `.system/tools/open_file.py` 就调用它，否则回报绝对路径。")
    ...
    def test_optional_marker_on_preceding_comment_passes(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            _mk_skill(
                ...
                'python3 .system/tools/open_file.py "<path>"\n```',
            )
            self.assertEqual(check_skill_system_independence(root), [])

    def test_control_plane_skill_is_exempt(self) -> None:
        """以 .system/ 为作业对象的 Skill 按定义不独立分发，由 frontmatter 自声明豁免。"""
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            _mk_skill(
                root, "forge", "运行 `python3 .system/tools/lint_workspace.py` 验证。",
    ...
    def test_private_untracked_skill_is_still_checked(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            d = root / ".system" / "skills" / "private"
            d.mkdir(parents=True)
            (d / ".gitignore").write_text("*\n", encoding="utf-8")
            _mk_skill(root, "private", '执行 `python3 .system/tools/open_file.py "<p>"`。')
            self.assertEqual(len(check_skill_system_independence(root)), 1)

    def test_self_path_inside_skills_is_not_flagged(self) -> None:
        """`.system/skills/<自身名>/…` 是安装位置示例，常与全局安装形式并列，不构成本项违规。"""
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            _mk_skill(root, "demo", "全局安装用 `<skill-dir>/scripts/x.py`，工作区内为 `.system/skills/demo/scripts/x.py`。")
            self.assertEqual(check_skill_system_independence(root), [])
