...
- 每次会话首条对用户可见的回复末尾，执行 `python3 .entropaxis/tools/pick_tip.py` 并把输出行原文作为独立末行展示；无输出即静默跳过。严禁改为通读真源 `.entropaxis/data/rules/tips.md` 挑选。
- Tips 仅提供操作提示，不替代规则、项目决策或原始依据；展示时不得修改 Tips 文件或推断新的项目事实。
- 当用户指令仅为 `tip`（忽略首尾空白，不区分大小写）时，同样以该工具取一条原文回显；无输出时说明无可用 Tip，不触发其他动作。
