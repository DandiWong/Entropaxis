...
> 📌 **数据隔离说明**：工作区实例数据（业务项目注册表、内部战略、凭据）独立存放于 `.entropaxis/data/`（嵌套于控制面之内，由 `.gitignore` 排除出版本库），严禁提交至公共系统仓库。

> ⚠️ **分发信道（唯一合法路径）**：把本系统交付给他人时，只允许 `git clone`、`git archive`、GitHub 版本库 ZIP 下载，或 `build_windows_installer.py` 产出的 Windows 安装包（其载荷即 `git archive` 跟踪集）。**严禁直接拷贝、压缩 `.entropaxis/` 目录或经网盘同步**——私有 Skill 靠自带 `.gitignore` 排除出版本库，`data/` 下的实例数据与凭据同样只靠 `.gitignore` 排除，二者在文件系统上依然物理存在，拷目录会把内部端点、业务口径与凭据一并带走。
...
装到已有工作区上即为升级：只覆盖 `.entropaxis/`，不动 `.entropaxis/data/`（你的实例数据与凭据），也不删你自己放进 `.entropaxis/skills/` 的私有能力。
...
- **根入口配置同步/自愈**：`python3 .entropaxis/tools/bootstrap.py`
- **角色模态与外部 Agent 交互配置**：`python3 .entropaxis/tools/setup_agents.py`（支持 `--scan` 探测 CLI、`--verify` 校验角色、`--apply-preset` 应用预设）
- **指令路由与静态上下文审计**：`python3 .entropaxis/tools/audit_routing.py --strict`（`--json` 输出完整 Hook、去重后的必需读取区间、未知项。静态估算不代表模型 usage 或费用）
- **工作区规则与健康度体检**：`python3 .entropaxis/tools/lint_workspace.py`（阻断项与建议项以工具输出为准；治理卫生类为建议项，全新工作区未初始化状态不红屏）
- **分发就绪核验（收件方视角）**：`python3 .entropaxis/tools/check_distribution.py`
- **构建 Windows 安装包**：`.github/workflows/build-windows-installer.yml` 在 `windows-latest` 上跑 `python3 .entropaxis/tools/build_windows_installer.py`（`--out` 产物路径、`--ref` 打包提交）导出版本库跟踪集载荷，再用 PyInstaller 把它与 `tools/install_windows.py` 一起冻结成单文件 exe；收件方侧由 `install_windows.py` 承接选目录、落盘与初始化
- **方案审计门禁校验**：`python3 .entropaxis/tools/check_audit_gate.py <审计报告路径>`
- **按配置打开文件**：`python3 .entropaxis/tools/open_file.py <path> [<path> ...]`
- **通用项目脚手架初始化**：`python3 .entropaxis/tools/init_project.py <项目名>`
- **软件应用脚手架初始化**：`python3 .entropaxis/tools/init_app.py <app-name> --target-dir <项目路径>/02_开发`
